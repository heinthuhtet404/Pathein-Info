...
if (isset($_GET['delete'])) {
    ...
    if ($id > 0) {
        $stmt = mysqli_prepare($db, "DELETE FROM clinics WHERE clinics_id = ?");
        mysqli_stmt_bind_param($stmt, "i", $id);
        if (mysqli_stmt_execute($stmt)) {
            header("Location: selectClinics.php?msg=deleted");
            exit;
        } else {
...
$total_result = mysqli_query($db, "SELECT COUNT(*) AS total FROM clinics");
$total_row = mysqli_fetch_assoc($total_result);
$total_pages = ceil($total_row['total'] / $limit);

$data = mysqli_query($db, "SELECT * FROM clinics ORDER BY clinics_id ASC LIMIT $offset,$limit");

// Get statistics
$total_clinics = $total_row['total'];

// Status column မရှိသေးတဲ့အတွက် 0 ထားလိုက်ပါတယ်
$active_clinics = 0;
$inactive_clinics = 0;
?>
...
<head>
    ...
    <title>Clinic List | Super Admin</title>
            ...
            <div class="ms-auto d-flex align-items-center gap-3">
                ...
                <div class="search-wrapper d-none d-md-block">
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control border-start-0" placeholder="Search clinics..." style="width: 200px;" id="searchInput">
                    </div>
                </div>
    ...
    <div class="container-fluid px-4 py-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div class="d-flex align-items-center gap-3">
                            <div class="header-icon">
                                <i class="fa-solid fa-hospital fs-1 text-white"></i>
                            </div>
                            <div>
                                <h2 class="fw-bold text-white mb-1">Clinic Management</h2>
                                <p class="text-white-50 mb-0">Manage and monitor all registered clinics</p>
                            </div>
                        </div>
                ...
                <div class="stats-card bg-white p-4 rounded-4 shadow-sm">
                    <div class="d-flex align-items-center gap-3">
                        <div class="stats-icon bg-primary bg-opacity-10 p-3 rounded-3">
                            <i class="fa-solid fa-hospital fs-3 text-primary"></i>
                        </div>
                        <div>
                            <span class="text-secondary text-uppercase small fw-semibold">Total Clinics</span>
                            <h3 class="fw-bold mb-0"><?= $total_clinics ?></h3>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="stats-card bg-white p-4 rounded-4 shadow-sm">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <span class="text-secondary text-uppercase small fw-semibold">Active Clinics</span>
                            <h3 class="fw-bold mb-0"><?= $active_clinics ?></h3>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="stats-card bg-white p-4 rounded-4 shadow-sm">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <span class="text-secondary text-uppercase small fw-semibold">Inactive Clinics</span>
                            <h3 class="fw-bold mb-0"><?= $inactive_clinics ?></h3>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <?php if (isset($_GET['msg']) && $_GET['msg'] == 'deleted'): ?>
            <div class="alert alert-success alert-dismissible fade show rounded-4 shadow-sm" role="alert">
                <i class="fa-solid fa-check-circle me-2"></i>Clinic has been deleted successfully!
            ...
            <div class="col-12">
                <div class="main-card bg-white rounded-4 shadow-lg overflow-hidden">
                    ...
                    <div class="card-header bg-white border-0 p-4">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <span class="badge bg-primary rounded-pill"><?= $total_clinics ?> entries</span>
                            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table custom-table mb-0" id="clinicTable">
                                ...
                                if (mysqli_num_rows($data) > 0) {
                                    ...
                                    while ($row = mysqli_fetch_assoc($data)) { ?>
                                        <tr class="align-middle">
                                            ...
                                            <td>
                                                <?php if (!empty($row['clinics_logo'])) { ?>
                                                    <div class="clinic-logo-wrapper">
                                                        <img src="uploads/<?= htmlspecialchars($row['clinics_logo']) ?>" 
                                                             class="clinic-logo" 
                                                             alt="Clinic Logo">
                                                    </div>
                                                <?php } else { ?>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <div>
                                                        <h6 class="fw-semibold mb-1"><?= htmlspecialchars($row['clinics_name']) ?></h6>
                                                        <small class="text-muted">ID: CL-<?= str_pad($row['clinics_id'], 4, '0', STR_PAD_LEFT) ?></small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    ...
                                                    <span><?= htmlspecialchars($row['clinics_email']) ?></span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    <i class="fa-solid fa-phone text-muted small"></i>
                                                    <span><?= htmlspecialchars($row['clinics_phone']) ?></span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    <i class="fa-solid fa-location-dot text-muted small"></i>
                                                    <span><?= htmlspecialchars($row['clinics_address']) ?></span>
                                                </div>
                                            </td>
                                            <td>
                                                <span class="description-text">
                                                    <?= htmlspecialchars(substr($row['clinics_description'] ?? '', 0, 50)) ?>
                                                    <?php if (strlen($row['clinics_description'] ?? '') > 50) echo '...'; ?>
                                                </span>
                                            </td>
                                            <td class="text-end pe-4">
                                                <div class="action-buttons">
                                                    <button class="action-btn view" onclick="viewClinic(<?= $row['clinics_id'] ?>)" 
                                                            data-bs-toggle="tooltip" title="View Details">
                                                        <i class="fa-solid fa-eye"></i>
                                                    </button>
                                                    <button class="action-btn edit" onclick="editClinic(<?= $row['clinics_id'] ?>)" 
                                                            data-bs-toggle="tooltip" title="Edit Clinic">
                                                        <i class="fa-solid fa-pen"></i>
                                                    </button>
                                                    <a href="?delete=<?= $row['clinics_id'] ?>" 
                                                       class="action-btn delete" 
                                                       onclick="return confirm('Are you sure you want to delete this clinic? This action cannot be undone.')"
                                                       data-bs-toggle="tooltip" title="Delete Clinic">
                                    ...
                                    <tr>
                                        <td colspan="8" class="text-center py-5">
                                            <div class="empty-state">
                                                <i class="fa-solid fa-hospital-circle-exclamation fa-4x text-muted opacity-50 mb-3"></i>
                                                <h5 class="text-muted">No clinics found</h5>
                                                <p class="text-muted small">Get started by adding your first clinic</p>
                                                <button class="btn btn-primary mt-3 rounded-pill" data-bs-toggle="modal" data-bs-target="#addClinicModal">
                                                    <i class="fa-solid fa-plus me-2"></i>Add Clinic
                                                </button>
                                            </div>
                                        </td>
                    ...
                    <div class="card-footer bg-white border-0 p-4">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                            <div class="text-muted small">
                                Showing <?= $offset + 1 ?> to <?= min($offset + $limit, $total_clinics) ?> of <?= $total_clinics ?> entries
                            </div>
                            <nav>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow-lg">
            ...
            <div class="modal-body p-4">
                <p class="text-muted mb-0">Clinic addition form will be implemented here.</p>
            </div>
            <div class="modal-footer border-0 pb-4 px-4">
                <button type="button" class="btn btn-light rounded-pill px-4" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-primary rounded-pill px-4">Add Clinic</button>
            </div>
        </div>
    </div>
</div>
...
document.getElementById('searchInput')?.addEventListener('keyup', function() {
    ...
    const tableRows = document.querySelectorAll('#clinicTable tbody tr');
...
function exportTable() {
    ...
    a.href = url;
    a.download = 'clinics_list.csv';
    a.click();
}

// Placeholder functions
function viewClinic(id) {
    alert('View clinic details for ID: ' + id);
}

function editClinic(id) {
    alert('Edit clinic for ID: ' + id);
}
